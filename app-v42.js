...
import {
  ...
  query, orderBy, onSnapshot, arrayUnion, arrayRemove, enableIndexedDbPersistence
...
let tripStartDate = null, tripEndDate = null; // 'YYYY-MM-DD', optioneel door de reiziger ingesteld
...
function obShowStep(id) {
  ...
  obClearError();
  if (id === 'obStepRole') renderSavedTrips('savedTripsOnboard', 'savedTripsOnboardList', null, false);
}

// ---------- Meerdere reizen: onthouden welke reizen dit toestel al kent ----------
function getSavedTrips() {
  try { return JSON.parse(localStorage.getItem('onderwegTrips') || '[]'); }
  catch (e) { return []; }
}
function saveTripToList(code, name, role) {
  const trips = getSavedTrips().filter(t => t.code !== code);
  trips.unshift({ code, name, role, lastVisited: new Date().toISOString() });
  localStorage.setItem('onderwegTrips', JSON.stringify(trips.slice(0, 10)));
}
function switchToSavedTrip(code) {
  localStorage.setItem('tripCode', code);
  location.reload();
}
// Vult een "eerdere reizen"-lijst op de opgegeven plek. excludeCode laat de
// huidige reis (indien van toepassing) weg uit de lijst — geen zin om naar
// jezelf te kunnen "wisselen".
function renderSavedTrips(wrapId, listId, excludeCode, removable) {
  const wrap = document.getElementById(wrapId);
  const list = document.getElementById(listId);
  if (!wrap || !list) return;
  const trips = getSavedTrips().filter(t => t.code !== excludeCode);
  ...
  trips.forEach((t) => {
    ...
    btn.type = 'button';
    btn.className = 'saved-trip-btn';
    btn.style.flex = '1';
    btn.innerHTML = `
      <span class="saved-trip-btn__name">${escapeHtml(t.name || t.code)}</span>
      <span class="saved-trip-btn__meta">${t.role === 'follower' ? '👀 volger' : '🧳 reiziger'} · ${escapeHtml(t.code)}</span>
    `;
    btn.addEventListener('click', () => switchToSavedTrip(t.code));
    row.appendChild(btn);
    if (removable) {
      const removeBtn = document.createElement('button');
      removeBtn.type = 'button';
      removeBtn.className = 'comment__remove';
      removeBtn.title = 'Vergeten';
      removeBtn.textContent = '×';
      removeBtn.addEventListener('click', () => {
        forgetSavedTrip(t.code);
        renderSavedTrips(wrapId, listId, excludeCode, removable);
      });
...
function forgetSavedTrip(code) {
  const trips = getSavedTrips().filter(t => t.code !== code);
  localStorage.setItem('onderwegTrips', JSON.stringify(trips));
}
...
function registerAsFollower(code) {
  const ref = doc(db, 'trips', code, 'followers', currentUser.uid);
  async function write() {
    let joinedAt = null;
    try {
      const snap = await getDoc(ref);
      if (snap.exists() && snap.data().joinedAt) joinedAt = snap.data().joinedAt;
    } catch (e) { /* kon niet lezen — dan zetten we 'm hieronder gewoon (opnieuw) */ }
    await setDoc(ref, {
      uid: currentUser.uid,
      name: authorName(),
      joinedAt: joinedAt || new Date().toISOString(), // blijft staan op het allereerste moment
      lastSeen: new Date().toISOString() // wordt wél elke heartbeat bijgewerkt
    });
  }
  write().catch((err) => {
...
async function enterApp(code, role) {
  ...
  const tripName = tripData.name || 'Jut en Juul op vakantie';

  tripStartDate = tripData.startDate || null;
  tripEndDate = tripData.endDate || null;
  document.getElementById('tripStartDateInput').value = tripStartDate || '';
  document.getElementById('tripEndDateInput').value = tripEndDate || '';
  ...
  saveTripToList(code, tripName, role);
  renderSavedTrips('savedTripsOverzicht', 'savedTripsOverzichtList', code, true);
  ...
  document.getElementById('inviteCodePanel').hidden = role === 'follower';
  document.getElementById('backupPanel').hidden = role === 'follower';
  document.getElementById('tripDatesPanel').hidden = role === 'follower';
  document.getElementById('stayForm').hidden = role === 'follower';
  updateWhoButton();
...
document.getElementById('btnSaveTripDates').addEventListener('click', async () => {
  if (currentRole === 'follower') return;
  const start = document.getElementById('tripStartDateInput').value;
  const end = document.getElementById('tripEndDateInput').value;
  if (start && end && start > end) { toast('De einddatum ligt vóór de begindatum'); return; }
  try {
    await setDoc(doc(db, 'trips', currentTripCode), { startDate: start || null, endDate: end || null }, { merge: true });
    tripStartDate = start || null;
    tripEndDate = end || null;
    toast('Reisdata opgeslagen ✓');
    renderDashboard();
  } catch (err) {
...
document.getElementById('btnDownloadBackup').addEventListener('click', () => {
  if (currentRole === 'follower') return;
  if (!allEntries.length) { toast('Nog geen momenten om te back-uppen'); return; }
  const backup = {
    tripName: document.getElementById('tripTitle').textContent,
  ...
  a.href = url;
  a.download = `onderweg-backup-${stamp}.json`;
...
// Veiligheidsklep: als er een opgeslagen reiscode is (dus het laadscherm
// wordt getoond), maar het inloggen om wat voor reden dan ook nooit tot een
// werkende reis leidt — bijv. een verlopen sessie — val dan na een paar
// seconden alsnog terug op het gewone keuzescherm, in plaats van iemand voor
// altijd op "Jullie reis wordt geladen…" te laten staren.
if (localStorage.getItem('tripCode')) {
  setTimeout(() => {
    if (document.getElementById('obStepLoading').hidden === false) {
      obShowStep('obStepRole');
    }
  }, 6000);
}
...
// ---------- Verblijven ----------
let allStays = [];
let unsubscribeStays = null;
function subscribeToStays(code) {
  if (unsubscribeStays) unsubscribeStays();
  const q = query(collection(db, 'trips', code, 'stays'), orderBy('startDate'));
  unsubscribeStays = onSnapshot(q, (snapshot) => {
    allStays = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
    renderStays();
  }, (err) => {
    const wrap = document.getElementById('staysList');
    if (wrap) wrap.innerHTML = `<p class="panel__hint" style="color:var(--brick);margin:0;">Kon verblijven niet laden: ${escapeHtml(err.message)}</p>`;
  });
}
function renderStays() {
  ...
  if (!wrap) return;
  if (!allStays.length) {
    wrap.innerHTML = '<p class="panel__hint" style="margin:0;">Nog geen verblijven toegevoegd.</p>';
    return;
  }
  const todayStr = new Date().toISOString().slice(0, 10);
  wrap.innerHTML = '';
  allStays.forEach((s) => {
    const isCurrent = s.startDate <= todayStr && (!s.endDate || s.endDate >= todayStr);
    const row = document.createElement('div');
    row.className = 'stay-row' + (isCurrent ? ' stay-row--current' : '');
    const datesText = s.endDate && s.endDate !== s.startDate
      ? `${fmtDayHeading(s.startDate)} — ${fmtDayHeading(s.endDate)}`
      : fmtDayHeading(s.startDate);
    row.innerHTML = `
      <span class="stay-row__icon">${isCurrent ? '📍' : '🏨'}</span>
      <div class="stay-row__body">
        <div class="stay-row__name">${escapeHtml(s.name)}</div>
        <div class="stay-row__dates">${datesText}</div>
      </div>
      ${currentRole !== 'follower' ? '<button type="button" class="comment__remove" title="Verwijderen">×</button>' : ''}
    `;
    const removeBtn = row.querySelector('.comment__remove');
    if (removeBtn) {
      removeBtn.addEventListener('click', () => {
        if (confirm(`"${s.name}" verwijderen?`)) {
          deleteDoc(doc(db, 'trips', currentTripCode, 'stays', s.id)).catch((err) => {
...
document.getElementById('btnAddStay').addEventListener('click', async () => {
  if (currentRole === 'follower') return;
  const name = document.getElementById('stayNameInput').value.trim();
  const start = document.getElementById('stayStartInput').value;
  const end = document.getElementById('stayEndInput').value;
  if (!name || !start) { toast('Vul in ieder geval een naam en begindatum in'); return; }
  if (end && end < start) { toast('De einddatum ligt vóór de begindatum'); return; }
  try {
    await setDoc(doc(collection(db, 'trips', currentTripCode, 'stays')), {
      name, startDate: start, endDate: end || null
    });
    document.getElementById('stayNameInput').value = '';
    document.getElementById('stayStartInput').value = '';
    document.getElementById('stayEndInput').value = '';
    toast('Verblijf toegevoegd ✓');
  } catch (err) {
    toast('Opslaan mislukt: ' + err.message);
  }
});
...
function renderFollowers() {
  ...
  allFollowers.forEach((f) => {
    ...
    const joinedText = f.joinedAt ? new Date(f.joinedAt).toLocaleString('nl-NL', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '';
    row.innerHTML = `
      <span class="author-dot" style="background:${authorColor(f.name)}">${escapeHtml(f.name.slice(0, 1).toUpperCase())}</span>
      <span style="flex:1;">
        ${escapeHtml(f.name)}
        ${joinedText ? `<span class="follower-row__joined">sinds ${joinedText}</span>` : ''}
      </span>
...
function toggleCommentLike(id, currentlyLiked) {
  const ref = doc(db, 'trips', currentTripCode, 'comments', id);
  updateDoc(ref, {
    likes: currentlyLiked ? arrayRemove(currentUser.uid) : arrayUnion(currentUser.uid)
  }).catch((err) => toast('Liken mislukt: ' + err.message));
}
...
function showView(id) {
  ...
  if (activeView) {
    ...
    void activeView.offsetWidth; // forceer reflow zodat de animatie elke keer opnieuw start
...
// Voor de geanimeerde achtergrond op de dagkop volstaat één "dominant"
// weertype — gewoon het eerst ingevulde weer die dag.
function getDayDominantWeather(dayEntries) {
  const found = dayEntries.find(e => e.weather);
...
function renderTimeline() {
  ...
  const searchEmptyEl = document.getElementById('timelineSearchEmpty');

  let entriesToShow = allEntries;
  if (query) {
    entriesToShow = allEntries.filter(e =>
  ...
  if (query && entriesToShow.length === 0) {
    ...
    Object.keys(groups).sort().reverse().forEach(day => {
      ...
      const dominantWeather = getDayDominantWeather(groups[day]);
      const labelText = fmtDayHeading(day) + (countryFlag ? ' ' + countryFlag : '') + (weatherIcons ? ' ' + weatherIcons : '');
      if (dominantWeather) {
        heading.dataset.weather = dominantWeather;
        heading.innerHTML = `<span class="day-heading__label">${escapeHtml(labelText)}</span>`;
      } else {
        heading.textContent = labelText;
      }
      container.appendChild(heading);
      groups[day].forEach(entry => container.appendChild(renderPostcard(entry)));
      if (!query) container.appendChild(renderDayComments(day));
    });
  }
...
function renderCommentRow(c, day, isReply, replyingToName) {
  ...
  row.innerHTML = `
    ...
    <div style="flex:1;">
      ...
      <div class="comment__actions">
        <button type="button" class="comment__like-btn${isLiked ? ' is-liked' : ''}">${isLiked ? '❤️' : '🤍'}${likes.length ? ' ' + likes.length : ''}</button>
  ...
  `;
  row.querySelector('.comment__like-btn').addEventListener('click', (e) => {
    toggleCommentLike(c.id, isLiked);
    popEffect(e.currentTarget);
  });
...
function renderDiary() {
  ...
  days.forEach((day, i) => {
    ...
    wrap.appendChild(renderDiaryChapter(day, dayEntries, computeDayNumberFor(day, i)));
  });
}
...
function renderDiaryChapter(day, dayEntries, dayNumber) {
  ...
  heading.innerHTML = `
    <p class="diary-chapter__daynum">${formatDayLabel(dayNumber)}${countryFlag ? ' ' + countryFlag : ''}${weatherIcons ? ' · ' + weatherIcons : ''}</p>
...
let pvScale = 1, pvTranslateX = 0, pvTranslateY = 0;
...
function pvResetZoom() {
  pvScale = 1; pvTranslateX = 0; pvTranslateY = 0;
  pvApplyTransform();
}
...
function renderPhotoViewer() {
  ...
  if (!p) return;
  pvResetZoom(); // nieuwe foto begint altijd weer op normale grootte
...
// Eigen knijp-/dubbeltik-zoom op de foto zelf — helemaal los van de systeem-
// paginazoom (die blijft uitgeschakeld voor het app-gevoel). Bij het inzoomen
// schakelt vegen-om-te-navigeren zichzelf uit; je sleept dan de foto rond.
(function setupPhotoViewerGestures() {
  const viewer = document.getElementById('photoViewer');
  const imgWrap = viewer.querySelector('.photo-viewer__img-wrap');
  ...
  imgWrap.addEventListener('touchstart', (e) => {
    if (e.touches.length === 2) {
      ...
      if (pvScale > 1) {
        ...
        panStartTX = pvTranslateX; panStartTY = pvTranslateY;
      }
      const now = Date.now();
      if (now - lastTapTime < 300) {
        if (pvScale > 1) { pvResetZoom(); } else { pvScale = 2.5; pvApplyTransform(); }
        lastTapTime = 0;
      } else {
        lastTapTime = now;
      }
  ...
  imgWrap.addEventListener('touchmove', (e) => {
    if (e.touches.length === 2) {
      ...
      pvScale = Math.min(4, Math.max(1, pinchStartScale * (dist / pinchStartDist)));
      pvApplyTransform();
    } else if (e.touches.length === 1 && isPanning) {
      pvTranslateX = panStartTX + (e.touches[0].clientX - panStartX);
      pvTranslateY = panStartTY + (e.touches[0].clientY - panStartY);
      pvApplyTransform();
    }
  }, { passive: true });

  imgWrap.addEventListener('touchend', () => { isPanning = false; }, { passive: true });

  // Vegen om naar de vorige/volgende foto te gaan — alleen als er niet is ingezoomd.
  viewer.addEventListener('touchstart', (e) => {
    if (e.touches.length === 1 && pvScale === 1) { startX = e.touches[0].clientX; }
  }, { passive: true });
...
function computeDayNumber() {
  // Als er een vaste startdatum is ingesteld, gebruik die als basis — anders
  // de datum van het allereerste vastgelegde moment.
  let raw;
  if (tripStartDate) {
    const todayStr = new Date().toISOString().slice(0, 10);
    raw = Math.max(1, Math.round((new Date(todayStr) - new Date(tripStartDate)) / 86400000) + 1);
  } else {
    const allDays = [...new Set(allEntries.map(e => e.timestamp.slice(0, 10)))].sort();
    if (!allDays.length) return null;
    const todayStr = new Date().toISOString().slice(0, 10);
    raw = Math.max(1, Math.round((new Date(todayStr) - new Date(allDays[0])) / 86400000) + 1);
  }
  // Niet verder tellen dan het einde van de reis — anders staat er onzin als
  // "Dag 19 van 11" zodra je de app lang na de reis nog een keer opent.
  const total = computeTotalDays();
  return total ? Math.min(raw, total) : raw;
}
// Is vandaag ná de ingestelde einddatum van de reis?
function isTripOver() {
  if (!tripEndDate) return false;
  return new Date().toISOString().slice(0, 10) > tripEndDate;
}
// Totaal aantal dagen van de reis, alleen bekend als beide reisdata zijn ingevuld.
function computeTotalDays() {
  if (!tripStartDate || !tripEndDate) return null;
  return Math.max(1, Math.round((new Date(tripEndDate) - new Date(tripStartDate)) / 86400000) + 1);
}
// Zet "Dag X" om in "Dag X van Y" zodra het totaal bekend is.
function formatDayLabel(dayNumber) {
  if (dayNumber == null) return '';
  const total = computeTotalDays();
  return `Dag ${dayNumber}${total ? ' van ' + total : ''}`;
}
// Voor het Reisdagboek: het "echte" dagnummer op basis van de kalenderdatum
// als er een startdatum is ingesteld, anders gewoon de hoeveelste dag met
// momenten (fallbackIndex) — zoals voorheen.
function computeDayNumberFor(dayStr, fallbackIndex) {
  if (tripStartDate) {
    return Math.max(1, Math.round((new Date(dayStr) - new Date(tripStartDate)) / 86400000) + 1);
  }
  ...
  wrap.innerHTML = `
    <div class="family-header">
      ...
      <div class="family-header__meta">
        ${dayNumber ? `<span>🗓️ ${formatDayLabel(dayNumber)}</span>` : ''}
...
function renderTodaySection() {
  ...
  if (todaysEntries.length === 0) {
    if (isTripOver()) {
      container.innerHTML = `
        <div class="today-card today-card--empty">
          <p class="today-card__eyebrow">🏁 Reis afgelopen</p>
          <p class="today-card__empty-msg">Deze reis zit erop — bekijk het Reisdagboek of de Tijdlijn om terug te kijken.</p>
        </div>
      `;
      return;
    }
    container.innerHTML = `
      <div class="today-card today-card--empty">
        <p class="today-card__eyebrow">☀️ Vandaag${dayNumber ? ' — ' + formatDayLabel(dayNumber) : ''}</p>
  ...
  const dominantWeather = getDayDominantWeather(todaysEntries);
  const eyebrowText = `☀️ Vandaag${dayNumber ? ' — ' + formatDayLabel(dayNumber) : ''}${countryFlag ? ' ' + countryFlag : ''}${weatherIcons ? ' ' + weatherIcons : ''}`;

  container.innerHTML = `
    <div class="today-card">
      <p class="today-card__eyebrow"${dominantWeather ? ` data-weather="${dominantWeather}"` : ''}>${dominantWeather ? `<span class="day-heading__label">${eyebrowText}</span>` : eyebrowText}</p>
...
// KAART
// ============================================================
let map = null, mapCluster = null, mapRouteLayer = null, tmLayer = null, mapLatLngs = [];
...
function rebuildMap() {
  ...
  // Genummerde pinnen die dicht bij elkaar liggen worden automatisch gebundeld
  // tot één "bolletje met aantal" — tik erop om in te zoomen. Voorkomt een
  // onleesbare kluwen pinnen zodra er veel momenten op één plek zijn.
  mapCluster = L.markerClusterGroup({ maxClusterRadius: 45, spiderfyOnMaxZoom: true, showCoverageOnHover: false }).addTo(map);
  mapRouteLayer = L.layerGroup().addTo(map);
  tmLayer = L.layerGroup(); // pas toegevoegd aan de kaart zodra de tijdmachine wordt geopend
  drawMapMarkers();
  map.invalidateSize();
...
function drawMapMarkers() {
  ...
  L.polyline(mapLatLngs, { color: '#B23A2E', weight: 2, dashArray: '6 6', opacity: 0.8 }).addTo(mapRouteLayer);
}
...
// terugspelen op de bestaande kaart. Gebruikt dezelfde map-instantie
...
function drawTimeMachineMarkers(entriesShown) {
  ...
  entriesShown.forEach((entry, i) => {
    ...
    const marker = L.marker([entry.lat, entry.lng], { icon }).addTo(tmLayer);
  ...
  if (latlngs.length > 1) {
    L.polyline(latlngs, { color: '#B23A2E', weight: 2, dashArray: '6 6', opacity: 0.8 }).addTo(tmLayer);
  }
...
function tmShowCallout(entry) {
  ...
  const photoHtml = (entry.photos && entry.photos.length)
    ? `<img src="${entry.photos[0]}" class="time-machine__callout-photo" alt="">`
    : '';
  el.innerHTML = `
    ${photoHtml}
    <div class="time-machine__callout-text">
      <b>${fmtDayHeading(entry.timestamp.slice(0, 10))} · ${fmtTime(entry.timestamp)}</b><br>${TYPE_ICON[type]} ${escapeHtml(entry.title)}${extra}
    </div>
  `;
  el.hidden = false;
...
function enterTimeMachine() {
  ...
  // Tijdens de tijdmachine tonen we losse, genummerde pinnen (geen bundeling) —
  // de gewone geclusterde route gaat tijdelijk aan de kant.
  if (mapCluster && map.hasLayer(mapCluster)) map.removeLayer(mapCluster);
  if (mapRouteLayer && map.hasLayer(mapRouteLayer)) map.removeLayer(mapRouteLayer);
  if (tmLayer && !map.hasLayer(tmLayer)) tmLayer.addTo(map);
  const slider = document.getElementById('tmSlider');
  slider.min = 0;
  slider.max = Math.max(0, tmDays.length - 1);
  tmPointer = -1; // begin helemaal bij het begin, nog niets in beeld
  tmRenderAtPointer(tmPointer, false);
  tmStartPlayback(); // en speel meteen af, geen handmatige actie nodig
}
function exitTimeMachine() {
  ...
  document.getElementById('btnToggleTimeMachine').textContent = '🕰️ Reis afspelen';
  if (map) {
    if (tmLayer && map.hasLayer(tmLayer)) map.removeLayer(tmLayer);
    if (mapCluster && !map.hasLayer(mapCluster)) mapCluster.addTo(map);
    if (mapRouteLayer && !map.hasLayer(mapRouteLayer)) mapRouteLayer.addTo(map);
    drawMapMarkers();
    if (mapLatLngs.length) map.fitBounds(mapLatLngs, { padding: [30, 30] });
  }
}
