...
function getDayCountryFlag(dayEntries) {
  const withLoc = dayEntries.filter(e => e.locationName);
  if (!withLoc.length) return '';
  const theme = countryThemeFor(withLoc[withLoc.length - 1].locationName);
  return theme ? theme.flag : '';
}
function getCurrentCountryTheme() {
  const withLoc = allEntries.filter(e => e.locationName).sort((a, b) => a.timestamp.localeCompare(b.timestamp));
  if (!withLoc.length) return null;
  return countryThemeFor(withLoc[withLoc.length - 1].locationName);
}
