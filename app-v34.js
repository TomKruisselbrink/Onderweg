...
function renderTimeline() {
  ...
  Object.keys(groups).sort().reverse().forEach(day => {
    ...
    heading.textContent = fmtDayHeading(day) + (weatherIcons ? ' ' + weatherIcons : '');
...
function renderDiaryChapter(day, dayEntries, dayNumber) {
  ...
  heading.innerHTML = `
    <p class="diary-chapter__daynum">Dag ${dayNumber}${weatherIcons ? ' · ' + weatherIcons : ''}</p>
...
function renderTodaySection() {
  ...
  container.innerHTML = `
    <div class="today-card">
      <p class="today-card__eyebrow">☀️ Vandaag${dayNumber ? ' — Dag ' + dayNumber : ''}${weatherIcons ? ' ' + weatherIcons : ''}</p>
